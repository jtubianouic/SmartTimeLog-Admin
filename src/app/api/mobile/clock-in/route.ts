...
export async function POST(request: Request) {
  ...
  const result = await recordAttendance(employee.employee_id, "clock_in", parsed.data);
  if (result.error === "invalid_transition") return apiError(409, "Employee is already clocked in.");
  if (result.error) return apiError(503, "Unable to record clock-in.");
