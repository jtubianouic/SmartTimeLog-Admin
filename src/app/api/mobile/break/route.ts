...
export async function POST(request: Request) {
  ...
  const result = await recordAttendance(employee.employee_id, "break", parsed.data);
  if (result.error === "invalid_transition") return apiError(409, "A break requires an active clock-in.");
  if (result.error) return apiError(503, "Unable to record break.");
