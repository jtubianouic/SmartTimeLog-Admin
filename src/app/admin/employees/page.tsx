...
import { Archive, RotateCcw, Search, Users } from "lucide-react";
...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const employees = (await getEmployees(q)).filter((employee) => (employee.isDeleted === true) === removed);
      ...
      <div className="glass-panel mt-6 overflow-x-auto rounded-lg">
        {employees.length ? (
          <table className="w-full min-w-[46rem] text-left text-sm">
            ...
            <tbody className="divide-y divide-white/6">{employees.map((employee) => <tr key={employee.employee_id}><td className="px-5 py-4 font-medium text-white">{employeeName(employee)}</td><td className="px-5 py-4 font-mono text-xs text-emerald-200">{employee.username}</td><td className="px-5 py-4 text-slate-400">{formatDateTime(employee.created_at)}</td><td className="px-5 py-4 text-slate-400">{formatDateTime(employee.updated_at)}</td><td className="px-5 py-4"><form action={setEmployeeDeleted}><input name="employee_id" type="hidden" value={employee.employee_id} /><input name="isDeleted" type="hidden" value={removed ? "false" : "true"} /><button className={`focus-ring flex items-center gap-2 text-xs ${removed ? "text-emerald-300 hover:text-emerald-200" : "text-rose-300 hover:text-rose-200"}`} type="submit">{removed ? <RotateCcw className="size-3.5" /> : <Archive className="size-3.5" />}{removed ? "Restore" : "Remove"}</button></form></td></tr>)}</tbody>
          </table>
